...
class TestFitShotDurations:
    ...
    def test_stretches_short_plans_toward_five_seconds(self):
        # 4 shots x 3s = 12s → stretch to 4 x 5s = 20s (exactly the floor)
    ...
    def test_partial_stretch_keeps_relative_weights(self):
        # 6 x 3s = 18s → only 2s deficit spread across 12s of headroom
        fitted, _ = _fit_shot_durations(_shots([3] * 6))
        total = sum(s["duration_s"] for s in fitted)
        assert TARGET_MIN_TOTAL_S - 0.2 <= total <= TARGET_MIN_TOTAL_S + 0.2
        assert all(s["duration_s"] < MAX_SHOT_RENDER_S for s in fitted)
    ...
    def test_drops_lowest_priority_shots_over_budget(self):
        # 8 x 5s = 40s → drop the trailing (lowest-priority) shot → 35s
        fitted, dropped = _fit_shot_durations(_shots([5] * 8))
        assert [s["index"] for s in dropped] == [8]
        assert sum(s["duration_s"] for s in fitted) <= TARGET_MAX_TOTAL_S
        assert [s["index"] for s in fitted] == [1, 2, 3, 4, 5, 6, 7]
...
class TestFitHeroDurations:
    def test_snaps_to_veo_grid_ties_round_up(self):
        fitted = _fit_hero_durations(_shots([5.0, 4.0, 3.0, 4.6]))
        assert [s["duration_s"] for s in fitted] == [6.0, 4.0, 4.0, 4.0]

    def test_six_five_second_shots_stay_under_ceiling(self):
        # 6 x 5s would bill 36s on Veo's grid — the tail shot drops to 4s.
        fitted = _fit_hero_durations(_shots([5.0] * 6))
        total = sum(s["duration_s"] for s in fitted)
        assert total <= TARGET_MAX_TOTAL_S
        assert all(s["duration_s"] in (4.0, 6.0) for s in fitted)
        # Downgrades come from the tail (plan order = priority).
...
class TestRenderVideoMultiShot:
    def test_five_shots_chained_and_concatenated(self, monkeypatch):
        ...
        # Fitted durations forwarded per shot
        assert all(r.duration_s == 4.0 for r in h.requests)
    ...
    def test_progress_is_monotonic_across_shots(self, monkeypatch):
        h = _Harness(monkeypatch)
        result = asyncio.run(render_video(_state([4, 4, 4, 4, 4])))
        assert result.get("status") != "failed"
