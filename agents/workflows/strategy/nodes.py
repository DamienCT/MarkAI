...
async def load_research(state: StrategyState) -> dict[str, Any]:
    ...
    return {"research_data": research.get("data", research)}
...
async def generate_positioning(state: StrategyState) -> dict[str, Any]:
    ...
    prompt = [
        {"role": "system", "content": "You are a brand strategist. Based on the research data, define a clear brand positioning. Return JSON with: value_proposition, differentiators, brand_voice, tone_attributes, key_messages."},
        {"role": "user", "content": f"Research data:\n{json.dumps(research, default=str)[:8000]}"},
    ]
...
async def define_pillars(state: StrategyState) -> dict[str, Any]:
    ...
    prompt = [
        {"role": "system", "content": "You are a content strategist. Define 4-6 content pillars for this brand. Each pillar should have: name, description, content_types, percentage_of_content, example_topics. Return a JSON array."},
        {"role": "user", "content": (
            f"Positioning:\n{json.dumps(state.get('positioning', {}), default=str)}\n\n"
            f"Research:\n{json.dumps(state.get('research_data', {}), default=str)[:5000]}"
        )},
    ]
    result = await chat_completion(prompt, temperature=0.5)
    try:
...
async def define_audiences(state: StrategyState) -> dict[str, Any]:
    ...
    prompt = [
        {"role": "system", "content": "You are a marketing strategist. Define 3-5 target audience segments. Each should have: segment_name, description, demographics, platforms, content_preferences, engagement_strategy, best_times. Return a JSON array."},
        {"role": "user", "content": (
            f"Positioning:\n{json.dumps(state.get('positioning', {}), default=str)}\n\n"
            f"Pillars:\n{json.dumps(state.get('pillars', []), default=str)}\n\n"
            f"Research:\n{json.dumps(state.get('research_data', {}), default=str)[:4000]}"
        )},
    ]
    result = await chat_completion(prompt, temperature=0.5)
    try:
...
async def plan_cadence(state: StrategyState) -> dict[str, Any]:
    ...
    prompt = [
        {"role": "system", "content": "You are a social media strategist. Create a posting cadence plan. Return JSON with platforms as keys, each having: posts_per_week, best_days, best_times, content_mix (mapping pillar to percentage)."},
        {"role": "user", "content": (
            f"Audiences:\n{json.dumps(state.get('audiences', []), default=str)}\n\n"
            f"Pillars:\n{json.dumps(state.get('pillars', []), default=str)}"
        )},
    ]
...
async def generate_themes(state: StrategyState) -> dict[str, Any]:
    ...
    prompt = [
        {"role": "system", "content": "You are a content strategist. Generate monthly themes for the next 3 months. Each month should have: month, theme_name, description, sub_themes, key_campaigns, pillar_focus. Return a JSON array."},
        {"role": "user", "content": (
            f"Positioning:\n{json.dumps(state.get('positioning', {}), default=str)}\n\n"
            f"Pillars:\n{json.dumps(state.get('pillars', []), default=str)}\n\n"
            f"Cadence:\n{json.dumps(state.get('cadence', {}), default=str)}"
        )},
    ]
