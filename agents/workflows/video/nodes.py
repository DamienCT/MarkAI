...
# Native multishot renders the WHOLE reel in one pass, so the per-call clamp
# above does not apply — forge segments go to 8s. The 3-5s regime produced
# "the scenes are too short and too random" (user, on a 7x4.3s reel): fewer,
# longer beats read as an edit with intent. 4x7.5s or 5x6s both land the 30s
# target inside the [4x5=20, 5x8=40] envelope.
NATIVE_MIN_SHOT_RENDER_S = 5.0
NATIVE_MAX_SHOT_RENDER_S = 8.0
NATIVE_MIN_PLAN_SHOTS = 4
NATIVE_MAX_PLAN_SHOTS = 5
...
async def plan_shots(state: VideoState) -> dict[str, Any]:
    ...
    try:
        ...
        # Native multishot pacing: the whole reel renders in ONE pass, so
        # the 3-5s per-call clamp is a chained-era relic there. Probed HERE
        # (one node before make_keyframe's probe) because the answer changes
        # what kind of script to ask for — a measured 7x4.3s reel drew "the
        # scenes are too short and too random" from the user, and the fix is
        # fewer, longer beats telling ONE continuous story.
        _tier = state.get("quality_tier") or "standard"
        native_pacing = False
        if _tier != "hero" and _config_settings.VIDEO_NATIVE_MULTISHOT:
            from shared.video import forge_supports_multishot

            native_pacing = await forge_supports_multishot()
        if native_pacing:
            shot_min, shot_max = NATIVE_MIN_SHOT_RENDER_S, NATIVE_MAX_SHOT_RENDER_S
            plan_min, plan_max = NATIVE_MIN_PLAN_SHOTS, NATIVE_MAX_PLAN_SHOTS
            render_desc = (
                "The whole reel is generated by an AI video model in ONE "
                "continuous pass — scene changes render as gentle "
                "cross-dissolves, so the shots must belong to one continuous "
                f"world. The finished reel runs {TARGET_TOTAL_S:.0f} seconds."
            )
            arc_block = (
                f"STORY ARC — plan {plan_min} to {plan_max} beats that tell "
                "ONE continuous story in ONE world (one location, one "
                "recurring person, one continuous quality of light), in this "
                "order:\n"
                "1. ESTABLISH + HOOK — the most inviting image of the "
                "product's world, already in motion. Atmosphere is allowed "
                "to breathe here; this beat earns the next three.\n"
                f"2. RITUAL — the product actually being used by the story's "
                "one recurring person: the gesture a buyer imagines "
                f"themselves making. ({reveal_beat})\n"
                "3. SENSORY PROOF — the closest, slowest beat: texture, "
                "colour, glisten, steam — the detail that proves the claim "
                "without a word.\n"
                "4. HUMAN PAYOFF — the same person (with company, if it "
                "helps) enjoying the result: the taste, the quiet smile, "
                "the shared table. This is the beat the viewer remembers.\n"
                f"Plan {plan_max} beats by splitting the ritual or adding a "
                "second distinct proof — never by repeating an action.\n\n"
                "CONTINUITY DISCIPLINE (one-pass model — non-negotiable):\n"
                "- Every beat is a DIFFERENT action. At most ONE pour in the "
                "whole reel. Never 'echo the opening composition' or return "
                "to an earlier setup — a story ends at its payoff, it does "
                "not loop.\n"
                "- The same person, location, light and props flow through "
                "every beat. Later beats may open mid-action, as the next "
                "moment of the same scene.\n"
            )
            discipline_open = (
                "SHORT-FORM DISCIPLINE (non-negotiable):\n"
                "- The product's world is in motion within seconds 0-2 — "
                "inviting, not static.\n"
                f"- Each shot is exactly ONE beat with one clear visual "
                f"change — it has to stay interesting for its full "
                f"{shot_min:.0f}-{shot_max:.0f} seconds, so give it internal "
                "motion (a pour, a hand, a slow camera move) and let it "
                "BREATHE — these are long, calm, premium beats, not cuts.\n"
            )
        else:
            shot_min, shot_max = MIN_SHOT_RENDER_S, MAX_SHOT_RENDER_S
            plan_min, plan_max = MIN_PLAN_SHOTS, MAX_SHOTS
            render_desc = (
                "Each shot in your list is generated as its own "
                f"{shot_min:.0f}-{shot_max:.0f} second clip by an AI video "
                "model and the clips are cut together into a "
                f"{TARGET_TOTAL_S:.0f} second reel."
            )
            arc_block = (
                f"STORY ARC — plan {plan_min} to {plan_max} beats that "
                "tell a COMPLETE marketing story, in this order:\n"
                "1. HOOK — the scroll-stopper: the product already in "
                "motion, the most arresting image you have.\n"
                ...
                "5. USE MOMENT — a real person actually using or enjoying "
                "it in its natural setting.\n"
                "6. PAYOFF — the emotional result: the satisfied face, the "
                "finished table, the restored calm.\n"
                "7. CTA — the closing frame that carries the call to "
                "action.\n"
                f"Plan {plan_min} beats by merging two adjacent stages, "
                f"{plan_max} by adding a second proof or use-moment beat. "
                f"Never return fewer than {plan_min} shots — a shorter list "
                "produces a reel that is too short to publish.\n"
            )
            discipline_open = (
                "SHORT-FORM DISCIPLINE (non-negotiable):\n"
                "- The product is VISIBLE and actively solving something "
                "within seconds 0-2. No slow establishing shots.\n"
                "- Each shot is exactly ONE beat with one clear visual "
                "change — it has to stay interesting for its full "
                f"{shot_min:.0f}-{shot_max:.0f} seconds, so give it "
                "internal motion (a pour, a hand, a camera move), never a "
                "static hold.\n"
                "- The final shot resolves back toward the opening "
        ...
        system = (
            ...
            "sentences).\n"
            + (
                "- NEVER write the product's name, the brand, the producer "
                "or any company name in prose — identify the product by "
                "colour, material and silhouette only (\"the olive-green "
                "glass bottle\"). A printed name in the prompt is exactly "
                "what comes back as invented lettering on the pack.\n"
                "- Keep the product's brief physical identity (colour/"
                "material) whenever it appears, and let the shared location "
                "and person carry continuity — later beats may open with a "
                "continuity phrase (\"Moments later,\" \"At the same "
                "table,\").\n"
                if native_pacing
                else
                "- Repeat the product and setting identity tags (what the "
                "product is, its colour/shape/material, where we are) in "
                "EVERY shot's prose — the model holds identity across "
                "scenes only when every scene restates it.\n"
            )
            ...
            "formality:\n"
            f"- Use the full {shot_min:.0f}-{shot_max:.0f} "
            ...
            f"Duration rules: return {plan_min} to {plan_max} shots. "
            f"Every duration_s is between {shot_min:.0f} and "
            f"{shot_max:.0f} seconds and the durations sum to about "
...
def _fit_shot_durations(
    shots: list[dict[str, Any]],
    *,
    min_shot_s: float = MIN_SHOT_RENDER_S,
    max_shot_s: float = MAX_SHOT_RENDER_S,
    ...
    20-35s window"): every duration is clamped to [min_shot_s, max_shot_s]
    and the 30s target is then distributed across the shots in proportion to
    their planned weights. The default range is the chained path's 3-5s
    per-provider-call clamp; the native multishot path passes its own 5-8s
    range — clamping a 4x7.5s native plan to 5s each would silently ship a
    20s reel. Shots are dropped ONLY when even a minimum-length reel would
    breach the hard TARGET_MAX_TOTAL_S ceiling (plan order = priority, so
    trailing shots go first).
    ...
    fitted = [
        {
            **s,
            "duration_s": min(
                max_shot_s,
                max(min_shot_s, float(s.get("duration_s") or 0.0)),
            ),
        }
        for s in shots
    ]
    dropped: list[dict[str, Any]] = []
    # Only a plan too long to fit even at min_shot_s loses beats.
    while len(fitted) > 1 and len(fitted) * min_shot_s > TARGET_MAX_TOTAL_S:
...
def _scrub_name_from_prose(prose: str, product_name: str) -> str:
    """Delete the product's printed name from native-multishot prose (pure).

    Under the packaging lock the name is both a supplier mention and the
    strongest label-bait in the prompt, and the delabel rewrite's "never name
    the brand" instruction demonstrably loses to a name the LLM sees in every
    input shot. The plan writes it as "<Name>, <appositive description>", so
    deletion leaves the description as the subject. Removes the FULL item
    name (comma-insensitive: "Emile Noel, Mild Olive Oil, 690g" also matches
    "Emile Noel Mild Olive Oil 690g") and the manufacturer head alone — but
    never a middle segment by itself, because "apricot jam" is legitimate
    description. Returns the original prose if scrubbing would empty it.
    """
    text = str(prose or "")
    ...
    if out == text:
        return text
    # Tidy the wound: doubled spaces/commas, separators stranded at sentence
    # starts, and the lowercase letter the deleted name used to precede.
    out = re.sub(r"\s{2,}", " ", out)
    out = re.sub(r",\s*,", ",", out)
    out = re.sub(r"(^|[.!?]\s+)[,;\s]+", r"\1", out)
    out = out.strip(" ,;")
    out = re.sub(
        r"(^|[.!?]\s+)([a-z])", lambda m: m.group(1) + m.group(2).upper(), out
    )
    return out or text
...
async def render_video(state: VideoState) -> dict[str, Any]:
    ...
    # The native decision has to precede the FITTING, not just the render:
    # native segments run 5-8s while chained provider calls clamp at 3-5s,
    # and fitting a 4x7.5s native plan with the chained range would silently
    # ship a 20s reel. make_keyframe already probed and left the answer on
    # the state; only a state built before that field falls back to probing
    # here (the later native gate reuses this same answer).
    _tier_early = state.get("quality_tier") or "standard"
    native_ready = False
    if _tier_early != "hero" and _config_settings.VIDEO_NATIVE_MULTISHOT:
        _capable = state.get("native_multishot_capable")
        if _capable is None:
            from shared.video import forge_supports_multishot

            _capable = await forge_supports_multishot()
        native_ready = bool(_capable)
    _fit_range: dict[str, float] = (
        {
            "min_shot_s": NATIVE_MIN_SHOT_RENDER_S,
            "max_shot_s": NATIVE_MAX_SHOT_RENDER_S,
        }
        if native_ready
        else {}
    )

    fitted, dropped = _fit_shot_durations(shots, **_fit_range)
    ...
    # Enforce the reel-length floor: too few shots can never reach the 20s
    # master-spec minimum, so split the longest beats in two and refit (the
    # refit re-clamps the halves and stretches them toward the range's top).
    # Native beats reach 8s, so 3 of them clear the floor; chained clips cap
    # at 5s and need MIN_RENDER_SHOTS.
    _min_render = 3 if native_ready else MIN_RENDER_SHOTS
    split_shots = len(fitted) < _min_render
    if split_shots:
        pre_split = len(fitted)
        fitted = _split_to_min_shots(fitted, _min_render)
        fitted, _ = _fit_shot_durations(fitted, **_fit_range)
        logger.info(
    ...
    if unverified_pack:
        ...
        # The rewrite is asked to drop the product name from prose, but an
        # LLM keeps structure it sees seven times: a measured reel shipped
        # every shot leading with "Emile Noel Mild Olive Oil, a golden olive
        # oil in a dark glass bottle, ..." — the name is both a supplier
        # mention and the strongest label-bait in the whole prompt. The plan
        # writes the name as "<Name>, <appositive description>", so deleting
        # the name mechanically leaves the description as a clean subject.
        _prod_name = str((state.get("product") or {}).get("name") or "")
        if _prod_name:
            for _shot in fitted:
                _shot["prose"] = _scrub_name_from_prose(
                    str(_shot.get("prose") or ""), _prod_name
                )
    ...
    if quality_tier != "hero" and _config_settings.VIDEO_NATIVE_MULTISHOT:
        # Same answer the fitter used — probed at most once per run.
        if native_ready:
            ...
            # The plan was fitted to native 5-8s beats, but every chained
            # provider clamps a single call to ~5s — rendering 6s requests
            # would ship ~5s clips under caption windows computed for 6s.
            # Refit to the chained range (splitting first if the shorter
            # clips can no longer reach the 20s floor).
            if len(fitted) < MIN_RENDER_SHOTS:
